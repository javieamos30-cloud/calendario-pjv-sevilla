...
async function fetchWorkbook(url){
  const candidates=[
    addParam(url,'download','1'),
    addParam(url,'action','download'),
    url
  ];
  ...
  for(const candidate of [...new Set(candidates)]){
    try{
      const res=await fetch(candidate,{
        redirect:'follow',
        headers:{
          'User-Agent':'Mozilla/5.0 PJV-Calendar-Sync/2.0'
        }
      });
      ...
      if(res.ok && b.length>4 && b[0]===0x50 && b[1]===0x4b){
        return b;
      }

      notes.push(`${res.status} ${ct} ${candidate}`);
    }catch(e){
      notes.push(`${e.message} ${candidate}`);
    }
  }

  throw new Error(
    `No he podido descargar el .xlsx desde EXCEL_URL. Comprueba que el vínculo sea público de solo lectura y que "Bloquear descarga" esté desactivado. Intentos: ${notes.join(' | ')}`
  );
}
...
function parseYear(ws){
  ...
  return ['SEPTIEMBRE','OCTUBRE','NOVIEMBRE','DICIEMBRE'].includes(name)
    ? 2026
    : 2027;
}
...
function normalizeTime(raw){
  ...
  if(range){
    return `${String(range[1]).padStart(2,'0')}:${range[2]}-${String(range[3]).padStart(2,'0')}:${range[4]}`;
  }
...
function extractPublic(raw){
  const whole=raw.match(/^\s*\[WEB\+\]\s*([\s\S]+)$/i);
  ...
  return raw
    .split(/\r?\n/)
    .map(s=>s.trim())
    .filter(s=>/^\s*(\[WEB\]|#WEB|🌐)/i.test(s))
    .map(s=>s.replace(/^\s*(\[WEB\]|#WEB|🌐)\s*/i,'').trim())
    .filter(Boolean);
}

function isYes(v){
  return ['SI','SÍ','YES','TRUE','1']
    .includes(text(v).trim().toUpperCase());
}
...
function sourceCellFor(ws, day, category){
  ...
  for(const c of CATEGORY_COLS){
    if(
      text(ws.getCell(2,c).value).trim().toUpperCase()
  ...
  for(let r=3;r<=ws.rowCount;r++){
    if(Number(ws.getCell(r,1).value)===day){
      return ws.getCell(r,col);
    }
  }
...
function buildFromControl(wb){
  ...
  for(let r=5;r<=control.rowCount;r++){
    if(!isYes(control.getCell(r,1).value)){
      continue;
    }

    const monthName=text(control.getCell(r,3).value)
      .trim()
      .toUpperCase();

    const category=text(control.getCell(r,4).value)
      .trim()
      .toUpperCase();

    if(!MONTHS.has(monthName) || !category){
      continue;
    }

    const ws=wb.getWorksheet(monthName);

    if(!ws){
      continue;
    }

    const day=dayFromExcelValue(control.getCell(r,2).value);
    ...
    const rawSource=sourceCell
      ? text(sourceCell.value).trim()
      : '';
...
function buildFromLegacyMarkers(wb){
  ...
  for(const ws of wb.worksheets){
    const sheetName=ws.name.toUpperCase().trim();

    if(!MONTHS.has(sheetName)){
      continue;
    }
    ...
    for(const c of CATEGORY_COLS){
      categories[c]=text(ws.getCell(2,c).value)
        .trim()
        .toUpperCase();
    }

    for(let r=3;r<=ws.rowCount;r++){
      const day=Number(ws.getCell(r,1).value);
      ...
      for(const c of CATEGORY_COLS){
        const raw=text(ws.getCell(r,c).value).trim();
...
async function build(){
  if(!EXCEL_URL){
    throw new Error('Falta EXCEL_URL en los Secrets de GitHub.');
  }
  ...
  const mode=events===null
    ? 'legacy'
    : 'control-web';

  if(events===null){
    events=buildFromLegacyMarkers(wb);
  }
  ...
  console.log(
    `Calendario generado (${mode}): ${events.length} eventos públicos.`
  );
}
